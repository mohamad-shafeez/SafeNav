...
SafeNav is a high-performance, decoupled Progressive Web App (PWA) designed to provide real-time travel safety analytics and personalized trip planning. It synthesizes live environmental data (Weather, AQI, Traffic) with Google’s Gemini AI to deliver context-aware safety directives tailored to individual user health profiles.
...
| Layer | Technologies |
| :--- | :--- |
| **Frontend** | Vanilla JavaScript (ES6+), HTML5, CSS3 (Custom Design System), Capacitor (Mobile Bridge) |
...
| **Security/Auth** | Firebase Admin SDK, JWT, Fernet Encryption (Cryptography.io) |
...
### 1. Dynamic Risk Calculation Algorithm
...
- **Exposure Multiplier**: Risk scales non-linearly with duration:
  - $30 \text{ min} = 1.0x$
  - $60 \text{ min} = 1.5x$
  - $120+ \text{ min} = 2.0x$
- **Adaptive Heat Thresholds**: Limits are dynamically set based on user profile (e.g., 31°C for cardiac/elderly vs. 38°C for standard).
- **Physical Exertion Penalty**: Physical activity (walking) in extreme temperatures (>30°C) or poor air quality (>70 AQI) adds a direct `+15 * duration_multiplier` heart-strain penalty.
- **AQI Sensitivity**: Triggers respiratory alerts at 55 AQI for asthma profiles vs. 100 AQI for standard users.
...
- **Incident Limit**: Critical hazards (Accidents, Fog, Lane Closures) are capped at **50 items** to prevent frontend browser lag during rendering.
